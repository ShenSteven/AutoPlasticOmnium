...
class PeakLin(QDialog, Ui_PeakLin):
    ...
    def ConsecutiveFrame(self, id, nad, sn, data, log=True, direction=PLinApi.TLIN_DIRECTION_PUBLISHER,
                         ChecksumType=PLinApi.TLIN_CHECKSUMTYPE_CLASSIC):
        time.sleep(self._interval / 1000)
        pci = '2' + sn
        self.SetFrameEntry(id, nad, pci, data, log, direction, ChecksumType)
        return True
        ...
        try:
            ...
            if self.SetFrameEntry(id, nad, pci, first_data):  # send first frame
                ...
                for i in range(quotient + 1):
                    ...
                    self.ConsecutiveFrame(id, nad, '{:X}'.format(j), consedata, False)
                    j = j + 1
                RSID = '76'
